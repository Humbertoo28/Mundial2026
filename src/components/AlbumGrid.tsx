...
export default function AlbumGrid({
  ...
  const [inventory, setInventory] = useState<Record<string, number>>(() => {
    ...
    initialUserStickers.forEach(us => {
      ...
      // También guardar con la clave normalizada (sin espacios, mayúsculas)
      const normalized = us.sticker_id.replace(/\s/g, '').toUpperCase();
      acc[normalized] = us.quantity;
    });
    // También mapear desde los IDs del catálogo hacia los del inventario
    initialStickers.forEach(s => {
      const normalizedCatalog = s.id.replace(/\s/g, '').toUpperCase();
      // Buscar si existe en el inventario con la forma original o la normalizada
